...
export default class ViewArchiveList extends Component {
 ...
 componentWillUnmount () {
  ...
  handleDelete(dataItem) {
    Realm.open(appSchema).then((realm) => {
      let deleteItem = realm.objects('tasks').filtered(`id = ${dataItem.item.id}`)
  ...
  render() {
    ...
    if (this.state.tasks) {
      tasks = this.state.tasks.objects('tasks').sorted('id', true).filtered(`archived = false AND number = '${this.props.navigation.getParam('task').number}'`)
    } else {
      tasks = []
    }
    ...
    return (
      <View style={style.screen}>
        ...
        <View style={{alignItems: 'center', margin: 5}}>
          {this.AvatarType(this.props.navigation.getParam('task', '-').fullname,this.props.navigation.getParam('task', '-').imageUri)}
        ...
        <TouchableOpacity onPress={() => {
          this.props.navigation.navigate('NewEntry', {newTaskInfo: {
            number: this.props.navigation.getParam('task', '-').number,
            fullname: this.props.navigation.getParam('task', '-').fullname
          }})
        }}>
          <View style={style.newEntry}>
            <Text style={{color: 'white', marginRight: 5}}>Add New Bike</Text>
        ...
        renderHiddenItem={(data, rowMap) =>
          <View style={style.renderHiddenItem}>
            ...
            <TouchableOpacity onPress={ _ => {
              this.props.navigation.navigate('ViewEntry', {task: data.item})
